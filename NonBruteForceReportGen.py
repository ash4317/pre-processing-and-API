...
def generateDataFrames(scoreDfs, allUsefulSheetAttr, allTemplateAttr, allSheetAttr):
    ...
    result1 = pd.concat(frames, sort=False)
    result1.index.name = "Termsheet Name"
    allDfs["Match & Miss-match Matrix"] = result1.to_json()
    ...
    result2 = pd.DataFrame(data=attrs, index=index)
    allDfs["Attributes from Sheet"] = result2.to_json()
    ...
    result3 = pd.DataFrame(data=attrs, index=index)
    allDfs["Attributes from Template"] = result3.to_json()
    ...
    result4 = pd.DataFrame(data=attrs, index=index)
    allDfs["All Attributes from Sheet"] = result4.to_json()
...
def getAttributes(doc, isSheet, keyTerms, debug=False):
    ...
    for table in doc.find_all("table"):
        if len(table.find_all("tr")) > 0:
            # Checking nature of the table
            rows = table.find_all("tr")
            if len(rows) > 1:
                first_row = list(map(lambda x: x.text, rows[0].find_all("td")))
                second_row = list(
                    map(lambda x: x.text, rows[1].find_all("td")))

                # Checking if even cell has tag in first row
                flag1 = False
                for x in first_row:
                    if "[RMV_" in x or "[RDG_" in x:
                        flag1 = True
                        break

                # Checking if all cells have tags in second row
                flag2 = True
                for x in second_row:
                    if not ("[RMV_" in x or "[RDG_" in x):
                        flag2 = False
                        break

                if not flag1 and flag2:
                    for attr in first_row:
                        attr = "".join(
                            [letter for letter in attr if letter.isalpha()]).lower()
                    ...
                    for row in rows:
                        ...
                        if debug:
                            ...
                            heading = "".join(
                                [letter for letter in cells[0].text if letter.isalpha()]).lower()
                            availableAttributes.add(heading)
                        elif isSheet or ("[RDG_" in row.text or "[RDSB_" in row.text):
                            cells = row.find_all("td")
                            if len(cells) < 1:
                                continue
                            else:
