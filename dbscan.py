...
def preprocess(textdict):
    ...
    for text in textdict:
        ...
        #cleandict[text] = [w for w in tokens  if not w in words]
...
def dbscan(tfidf, flist):
    x = 0.29
    y = 2
    db = DBSCAN(eps=x, min_samples=y, metric='cosine').fit(tfidf)
    clusters = db.labels_.tolist()

    labeling={'Idea':flist, 'Cluster':clusters} #Creating dict having doc with the corresponding cluster number.
    frame=pd.DataFrame(labeling,index=[clusters], columns=['Document','Cluster']) # Converting it into a dataframe.
    print("eps=",x," min_samples=",y)
    print("\n")
    print(frame['Cluster'].value_counts(), end="; ") #Print the counts of doc belonging to each cluster.
    return frame
...
print(df.shape)
cdf = dbscan(df, flist)
#cdf.to_csv("dbscan_Interest_Rates.csv")
...
print(df.shape)
cdf = dbscan(df, flist)
#cdf.to_csv("dbscan_Bullet Participation.csv")
...
print(df.shape)
cdf = dbscan(df, flist)
#cdf.to_csv("dbscan_kmeans_Snowball.csv")
