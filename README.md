...
```
##### Input arguments-
1. filepath (optional)- The path of the excel sheet where all the URLs where the htm files are located. If filepath is given, then API will extract first from the URLs present in the excel sheet given in the filepath and then pre-process it. If filepath is not given, API will assume that data is extracted first (which is stored in the file "extract.json") and directly pre-process it.              
...
print(r.text)
```
Returns JSON object of the form given below (for 4 clusters):
```
{
    "Cluster 0": 11,
    "Cluster 1": 3,
