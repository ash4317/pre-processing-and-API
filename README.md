...
```
Input arguments- 
1) filepath- The path of the excel sheet where all the URLs where the htm files are located. Format of the excel sheet should be- (ISIN, URL)
2) no_of_docs (optional)- Number of URLs from which data is to be extracted. By default, it will extract from ALL the URLs.
...
```
Input arguments- 
1. filepath (optional)- The path of the excel sheet where all the URLs where the htm files are located. If filepath is given, then
...
Input arguments- 
1) filepath (optional)- Path of excel sheet or csv file where the pre-processed data is stored. If argument is not given, the API will assume that the pre-processed data is present in the file "preprocess.json" in the form of a list containing a dictionary like {ISIN, URL, Data} as individual elements.
2) k- Value of k in k-means algorithm which is the number of clusters to be made
3) thresh (optional)- The threshold value of variance in the variance threshold algorithm for reducing number of features. Default value is 0.001
4) pca_comp (optional)- The number of components in the Principal Component Analysis algorithm to again, reduce the features. Default value is 0.8
5) format (optional)- Output format of file where clustered data is exported. Valid values are "excel" and "csv". Default value is "excel".
...
Input arguments- 
1) filepath (optional)- Path of excel sheet or csv file where the pre-processed data is stored. If argument is not given, the API will assume that the pre-processed data is present in the file "preprocess.json" in the form of a list containing a dictionary like {ISIN, URL, Data} as individual elements.
2) eps- Value of epsilon in DBSCAN algorithm i.e, the maximum distance between two samples for one to be considered as in the neighborhood of the other.
3) min- The number of samples (or total weight) in a neighborhood for a point to be considered as a core point. This includes the point itself.
4) thresh (optional)- The threshold value of variance in the variance threshold algorithm for reducing number of features. Default value is 0.001
5) pca_comp (optional)- The number of components in the Principal Component Analysis algorithm to again, reduce the features. Default value is 0.8
6) format (optional)- Output format of file where clustered data is exported. Valid values are "excel" and "csv". Default value is "excel".
